...
Original file is located at
    https://colab.research.google.com/drive/1LvTmBHTRbU9rRChypkEBu936xV9WJbnX
"""
...
# Assuming you have a regression dataset with input features of shape (batch_size, channels, rows, columns)
in_chans = 2  # Number of input channels in your data
num_classes = 2  # Regression task, so there's only one output node

embed_dims: List[int] = [32, 64]  # List of embedding dimensions for each layer in the FAT model
depths: List[int] = [3, 3]  # List of depths for each layer in the FAT model
kernel_sizes: List[int] = [3, 3]  # List of kernel sizes for the attention mechanism
num_heads: List[int] = [4, 4]  # List of the number of attention heads for each layer
window_sizes: List[int] = [7, 7]  # List of window sizes for the attention mechanism
mlp_kernel_sizes: List[int] = [1, 1]  # List of kernel sizes for the MLP (feedforward) layer
mlp_ratios: List[float] = [4.0, 4.0]  # List of MLP expansion ratios

drop_path_rate = 0.1  # Drop path rate for stochastic depth regularization
use_checkpoint = False  # Whether to use gradient checkpointing for memory efficiency

# Instantiate the FAT model
model = FAT(
    in_chans=in_chans,
...
model.load_state_dict(torch.load('/content/model_mea_0.9478779.pth',map_location=torch.device('cpu')))
mae = nn.L1Loss()
model.eval();

array = torch.from_numpy(np.load("/content/totestFID32.npy"))
